...
function exp_label_Courbes_de_niveaux_1m_8_eval_expression(context) {
    // if(ALTITUDE % 5 = 0, ALTITUDE, NULL)

    var feature = context.feature;
    
    if (feature.properties) {
        return fnc_if([((feature.get('ALTITUDE')  % 5) == 0),feature.get('ALTITUDE') ,null], context);
    } else {
        return fnc_if([((feature.get('ALTITUDE')  % 5) == 0),feature.get('ALTITUDE') ,null], context);
    }
}
...
function exp_label_Route_d_acces_14_eval_expression(context) {
    // Nom
...
function exp_Arbres_remarquables_15rule0_eval_expression(context) {
    ...
    if (feature.properties) {
        return (fnc_attribute([fnc_get_feature(['Table_botanique_complet','fid',feature.get('id_botanique_ref') ], context),'ref_id_type'], context) == 2);
    } else {
        return (fnc_attribute([fnc_get_feature(['Table_botanique_complet','fid',feature.get('id_botanique_ref') ], context),'ref_id_type'], context) == 2);
    }
}